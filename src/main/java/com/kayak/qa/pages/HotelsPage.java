...
public class HotelsPage  extends TestBase{
	...
	//Initializing the Page Objects:
		...
		public void clickStartDateField() throws InterruptedException {
			Thread.sleep(5000);
			//WebElement element	=driver.findElement(By.xpath("//div[contains(@id,'-dateRangeInput-display-start') and contains(@aria-label,'Check-in date input')][1]"));
		
			//element.click();
			//Thread.sleep(5000);
			//element.sendKeys("03/04/2020");
			...
			actions.click();
			actions.build().perform();
			WebElement input2 = driver.findElement(By.xpath("//div[contains(@class,'_ia1 _iBD _iai')]/div[contains(@id,'-checkOut-input')][1]"));
			//input2.clear();
			input2.sendKeys("03/08/2020");
			input2.sendKeys(Keys.ENTER);
			//startdatelabel6.sendKeys("03/04/2020");
			
			List<WebElement> month_list= driver.findElements(By.xpath("//div[contains(@id,'-dateRangeInput-display-start') and contains(@aria-label,'Check-in date input')]"));
		...
		//for testing purpose
		public void handellist(){
			//boot StrapDownloads-Ul/li/div values
			WebElement pDropDown = driver.findElement(By.xpath("//input[@aria-label='Destination input']"));
	        pDropDown .sendKeys("BCN");
	      List <WebElement>   list = driver.findElements(By.xpath("//ul[contains(@role,'listbox')]/li/div/following-sibling::div"));
	       System.out.println(list.size()); // it will return the size of the list
	       for( int i=0; i<list.size(); i++) {		 
	          		System.out.println(list.get(i).getText());
	         if(list.get(i).getText().contains("Barcelona, Spain")) {
	          	  list.get(i).click();
	          	System.out.println("Vlues slected");
	          		break;		
	          		 } 
	          			}	
